...
from utils import (
    fetch_stock_data, add_moving_averages, build_lag_features,
...
def walk_forward_cv(close_prices: np.ndarray, lookback: int,
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(tscv.split(close_prices)):
        ...
        # ── Split-safe scaler ──────────────────────────────────────────────
        train_raw = close_prices[train_idx]
        # For validation, include lookback overlap from end of train
        val_start = max(0, val_idx[0] - lookback)
        val_raw   = close_prices[val_start: val_idx[-1] + 1]

        scaler       = MinMaxScaler(feature_range=(0, 1))
        train_scaled = scaler.fit_transform(train_raw.reshape(-1, 1))
        val_scaled   = scaler.transform(val_raw.reshape(-1, 1))
...
def main():
    ...
    with mlflow.start_run(run_name=f"{args.ticker}_{date.today()}"):
        ...
        cv_results = walk_forward_cv(
            close_prices.flatten(), args.lookback,
        ...
        xgb_metrics = compute_metrics(
            actual_test[:n_common], xgb_preds[:n_common], naive_preds[:n_common])

        # Naive
        naive_metrics = compute_metrics(
            actual_test[1:], naive_preds, naive_preds)
